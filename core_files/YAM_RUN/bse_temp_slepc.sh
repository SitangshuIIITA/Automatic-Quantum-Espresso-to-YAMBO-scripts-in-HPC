...
#       # 1. BSE spectra using slepC solver is computed at different temperatures. 
	...
	#-----------------------------------------------BSE + GW + phonon Calculation: SlepC solver------------------------------#
		...
		#----------------------------GW + temperature ELPH data merging-----------------------------------------------------#
                                               ...
                                               yambo
                                               yambo -r -X s -optics b -kernel sex -Ksolver s -V all -F $prefix.BSE-${line}$Kelvin.slpc.in
               ...
               #------------------------------COULOMB cut-off & RIM------------------------------#
                                                ...
                                                sed '55s/.*//' $prefix.BSE-${line}$Kelvin.slpc.in > 4
                                                sed '55 r 3' 4 > 5
                                                sed '55d' 5 > 6
                                                mv 6 $prefix.BSE-${line}$Kelvin.slpc.in
                                                rm -rf 1 2 3 4 5

                                                sed -i 's/RandQpts=0/RandQpts=1000000/g'                                $prefix.BSE-${line}$Kelvin.slpc.in
                                                sed -i 's/RandGvec= 1/RandGvec= 100/g'                                  $prefix.BSE-${line}$Kelvin.slpc.in
                                                sed -i 's/CUTGeo= "none"/CUTGeo= "box z"/g'                             $prefix.BSE-${line}$Kelvin.slpc.in
               ...
               #---------------------------FFT-GVecs and other Response size block---------------#
                                                ...
                                                sed -i.bak -e '30d'                                                     $prefix.BSE-${line}$Kelvin.slpc.in
                                                sed -i.bak "30i FFTGvecs= $fftgvecs"                                    $prefix.BSE-${line}$Kelvin.slpc.in
                                                
                                                sed -i.bak -e '173d'                                                    $prefix.BSE-${line}$Kelvin.slpc.in
                                                sed -i.bak "173i NGsBlkXs= $ngsblks"                                    $prefix.BSE-${line}$Kelvin.slpc.in

                                                sed -i.bak -e '81d'                                                     $prefix.BSE-${line}$Kelvin.slpc.in
                                                sed -i.bak "81i BSENGBlk= $bsengblk"                                    $prefix.BSE-${line}$Kelvin.slpc.in

                                                sed -i.bak -e '79d'                                                     $prefix.BSE-${line}$Kelvin.slpc.in
                                                sed -i.bak "79i BSENGexx= $bsengexx"                                    $prefix.BSE-${line}$Kelvin.slpc.in



                #----------------------------In-plane electric field directions-------------------#             

                                                sed -i '136s/ 1.000000 | 0.000000 | 0.000000 |/ 1.000000 | 1.000000 | 0.000000 |/g'     $prefix.BSE-${line}$Kelvin.slpc.in
                                                sed -i '183s/ 1.000000 | 0.000000 | 0.000000 |/ 1.000000 | 1.000000 | 0.000000 |/g'     $prefix.BSE-${line}$Kelvin.slpc.in
		...
		#----------------------------Include Bose Temperature-----------------------------#
						...
						sed -i.bak -e '131d'                                                $prefix.BSE-${line}$Kelvin.slpc.in
                                                sed -i.bak "131i 0.0100000 | 0.0100000 |         eV"                $prefix.BSE-${line}$Kelvin.slpc.in
		...
		#---------------------Includes GW + temperature dependent band gap energies--------#
               	...
               	#------------------------Reading the k point limits, max valence band, min conduction band from r_setup file-------------#
                                                ...
                                                sed -i 's/$/|/' 9
                                                sed -i.bak '116s/.*//' $prefix.BSE-${line}$Kelvin.slpc.in
                                                sed -i.bak '116 r 9' $prefix.BSE-${line}$Kelvin.slpc.in
                                                sed -i.bak '116d' $prefix.BSE-${line}$Kelvin.slpc.in
                ...
                #----------------------Includes photon energy range, W screening bands, writes exciton wavefunctions--------------------#
                                                ...
                                                sed -i '128s/0.00000 | 10.00000 |/0.00000 | 7.00000 |/g'                $prefix.BSE-${line}$Kelvin.slpc.in
                                                sed -i 's/BEnSteps= 100/BEnSteps= 500/g'                                $prefix.BSE-${line}$Kelvin.slpc.in
		...
		#-------------------------SLEPC solver: first 5 excitons----------------------------------------------------------------#
                                                ...
                                                $MPIRUN_YAMBO yambo -F $prefix.BSE-${line}$Kelvin.slpc.in -J output -C Report


                              cd Report

                                               mv o-output.alpha_q1_slepc_bse $prefix.${line}$Kelvin.o-output.alpha_q1_slepc_bse
                                               mv o-output.eps_q1_slepc_bse $prefix.${line}$Kelvin.o-output.eps_q1_slepc_bse
                                               mv o-output.jdos_q1_slepc_bse $prefix.${line}$Kelvin.o-output.jdos_q1_slepc_bse
